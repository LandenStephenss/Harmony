...
const guildCreate = (client, data, shard) => {
  const guild = Guild.create(data, shard.id, client);
...
const guildMemberAdd = (client, data) => {
  if (!client.users.has(data.user.id)) {
    client.users.set(data.user.id, data.user);
  }
  const member = Member.create(data, client);
  const guild = client.guilds.get(data.guild_id);
...
const messageCreate = (client, data) => {
  if (!client.users.has(data.author.id)) {
    client.users.set(data.author.id, data.author);
  }
...
const voiceStateUpdate = (client, data) => {
  ...
  member.voiceState = data.channel_id && data;
  return [data];
};
