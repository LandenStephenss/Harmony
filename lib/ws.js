...
const WeebSocket = typeof window === 'undefined' ? require('ws') : WebSocket;
...
try {
  ...
  write('Found Erlpack. Using ETF instead of JSON encoding', 2);
  encode = erlpack.pack;
 ...
 * @arg {number} [version] Gateway version
 * @returns {Promise<object>}
 */
const connect = async (token, shardCount, client, version) => {
  ...
  const url = `${g.url}?encoding=${encoding}&v=${version || GATEWAY_VERSION}`;
  x.shards = new Array(count);
  write(`Spawning ${count} shards`, 0);
  spawnShard(token, x, url, 0);
  return x;
};
...
const sendWS = (ws, op, d) => ws.send(encode({ d, op }));

const spawnShard = (token, client, url, id) => {
  const ws = new WeebSocket(url);
